...
import {
  ...
  getFormsChildPage,
  USCIS_FORMS_HUB,
} from "@/lib/uscisFormsCluster";

export function generateStaticParams() {
  return formsChildSlugs.map((slug) => ({ slug }));
}
...
export function generateMetadata({
  ...
  const page = getFormsChildPage(params.slug);
  if (!page) return {};
  return pageMetadata({
    title: page.seoTitle ?? page.title,
    description: page.metaDescription ?? page.excerpt,
    path: `/uscis/forms/${page.slug}`,
    type: "article",
    openGraph: {
...
export default function UscisFormChildPage({
  params,
}: {
  params: { slug: string };
}) {
  ...
  const articleJsonLd = {
    "@type": "Article",
    "@id": `${absoluteUrl(`/uscis/forms/${page.slug}`)}#article`,
    headline: page.title,
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": absoluteUrl(`/uscis/forms/${page.slug}`),
    },
    url: absoluteUrl(`/uscis/forms/${page.slug}`),
    inLanguage: "en-US",
  ...
  const siblings = formsChildPages.filter((p) => p.slug !== page.slug);
          ...
          <Container>
            <div className="mx-auto max-w-[720px]">
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-xs text-ink-400">
                ...
                <span className="inline-block rounded-full bg-blue-50 px-2.5 py-1 text-xs font-bold text-blue-700">
                  {page.formNumber}
                </span>
          ...
          <Container>
            <div className="mx-auto">
              ...
              <div className="mx-auto mt-6 max-w-[720px] text-sm">
                <Link
                  href={USCIS_FORMS_HUB}
                  className="font-medium text-brand-600 hover:text-brand-700"
                >
                  ← Back to USCIS Forms guide
                </Link>
              </div>
            </div>
          </Container>
