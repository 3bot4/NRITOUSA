...
function assess(
  ...
  if (faComparison === "all-current") {
    return {
      ...
      explanation: [
        ...
        `${bulletin.month} ${bulletin.year} Final Action Date for ${category} ${isIndia ? "India" : "Other"}: Current (C) — no priority-date cutoff applies this month for this chart`,
        ...staleNote,
      ],
      whatItMeans: `${category} ${isIndia ? "India" : "Other"} is marked "Current" in the ${bulletin.month} ${bulletin.year} bulletin — the chart imposes no priority-date cutoff for this category this month, so your date is current for Final Action purposes. Current status does not by itself grant a green card: final approval still requires meeting all other eligibility, admissibility, documentation, and processing requirements.`,
      chartGuide: "Since the category shows Current (C), no priority-date cutoff applies this month under either chart. Confirm with your attorney which chart USCIS authorizes, check USCIS processing times, and prepare to file or finalize I-485 if you otherwise qualify.",
      questionsForLawyer: [
  ...
  if (faComparison === "current") {
    return {
      ...
      chartGuide: "Your date appearing current under Table A means USCIS may be able to approve your green card once all other requirements are met. If I-485 is not yet filed, this may be the time to file — confirm timing, eligibility, and the authorized chart with your attorney. If pending, USCIS should be processing.",
      questionsForLawyer: [
  ...
  if (faComparison === "not-current" && dffComparison === "current") {
    return {
      ...
      explanation: [
        ...
        `This means you may be able to FILE I-485 this month — and then apply for an EAD and Advance Parole — if USCIS authorizes Dates for Filing. Check uscis.gov/visabulletininfo.`,
        ...staleNote,
      ],
      whatItMeans: `Your priority date of ${pdLabel} appears to be in the "Dates for Filing" window for ${bulletin.month} ${bulletin.year}: past the Final Action Date (you cannot be approved yet), but before the Dates for Filing cutoff (you may be able to file I-485 if USCIS authorizes Table B this month). Once an I-485 is filed, you can apply for an EAD and Advance Parole — these are separate applications (Forms I-765 and I-131) and are not granted automatically.`,
      chartGuide: "Filing under Dates for Filing (Table B) lets you apply for an EAD and Advance Parole (separate applications, not automatic), but your green card won't be approved until the Final Action Date (Table A) is also current. Verify USCIS has authorized Table B at uscis.gov/visabulletininfo before filing.",
      questionsForLawyer: [
  ...
  if (faComparison === "unavailable") {
    return {
      ...
      whatItMeans: `No immigrant visa number is available for final issuance in this category and country under this chart this month, regardless of priority date. This can happen near the end of the fiscal year or when demand has consumed the available numbers. Whether another filing or case step is available depends on your process stage and the chart USCIS authorizes — check the next month's bulletin and confirm with your attorney.`,
      chartGuide: "Unavailable (U) means no immigrant visa number is available for final issuance in this category under this chart this month, regardless of priority date. Other petition or case-maintenance steps may still be possible — monitor future months and confirm your options with your attorney.",
      questionsForLawyer: [
